...
static uint16_t *alloc_pixels(void) {
    const size_t bytes = (size_t)MAP_BG_SIZE * MAP_BG_SIZE * sizeof(uint16_t);
#ifdef ARDUINO
...
uint16_t *map_bg_back_buffer(void) { return s_back; }
...
void map_bg_commit(double lat, double lon, float rangeKm) {
    ...
    s_lat = lat; s_lon = lon; s_rangeKm = rangeKm; ++s_version;
}
...
bool map_bg_front(const uint16_t **pixels, double *lat, double *lon,
    ...
    if (lat) *lat = s_lat;
    if (lon) *lon = s_lon;
    if (rangeKm) *rangeKm = s_rangeKm;
    if (version) *version = s_version;
    return true;
}

void map_bg_clear(void) {
    std::lock_guard<std::mutex> lock(s_mutex);
    s_version = 0;
}
