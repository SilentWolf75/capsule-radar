...
static void handleRoot() {
    ...
    const int needed = snprintf(buf, BUFSZ,
        ...
        ".dot-box{position:relative;width:52px;height:52px;border-radius:50%%;padding:2px;"
        "background:linear-gradient(145deg,#1b3829,#06140c);box-shadow:0 0 16px rgba(29,255,134,.35)}"
        ".dot{position:relative;width:100%%;height:100%%;border-radius:50%%;background:#000;overflow:hidden;border:1px solid rgba(29,255,134,.7)}"
        ".dot::before{content:'';position:absolute;inset:0;animation:sw 3s linear infinite;"
        "background:conic-gradient(from 0deg,rgba(29,255,134,.65),rgba(29,255,134,.08) 50deg,transparent 65deg)}"
        ".dot-ring{position:absolute;inset:4px;border-radius:50%%;border:1px stroke rgba(29,255,134,.28);pointer-events:none}"
        ".dot-blip{position:absolute;width:4px;height:4px;background:#1dff86;border-radius:50%%;box-shadow:0 0 5px #1dff86;animation:bp 3s ease-out infinite}"
        ".b1{top:30%%;left:65%%;animation-delay:.5s}.b2{top:65%%;left:32%%;animation-delay:1.5s}"
        "@keyframes sw{to{transform:rotate(360deg)}}"
        "@keyframes bp{0%%{opacity:0;transform:scale(.6)}10%%{opacity:1;transform:scale(1.3)}25%%{opacity:.9}100%%{opacity:.15}}"
        ...
        "</style></head><body>"
        "<div class=hd><div class=dot-box><div class=dot><div class='dot-blip b1'></div><div class='dot-blip b2'></div></div></div><div><h1>Capsule Radar</h1><p class=sub>Live ADS-B radar &middot; configuration</p></div></div>"
